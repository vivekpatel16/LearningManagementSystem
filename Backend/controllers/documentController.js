...
exports.createDocument = asyncHandler(async (req, res) => {
  ...
  const highestOrder = await ChapterContent.findOne({ chapter_id }).sort({ order: -1 }).lean();
  const newOrder = highestOrder ? highestOrder.order + 1 : 1;
...
exports.updateDocument = asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    ...
    if (document.pdf_url.includes('cloudinary.com')) {
      const publicId = document.pdf_url.split('/').pop().split('.')[0];
      try {
        await cloudinary.uploader.destroy(`lms-documents/${publicId}`, { resource_type: "raw" });
      } catch (err) {
        console.error("Failed to delete old Cloudinary PDF:", err);
      }
    }
    ...
    document.pdf_url = cloudResult.secure_url;
  }
...
// @route   POST /api/documents/:id/progress
// @access  Private
exports.trackDocumentProgress = asyncHandler(async (req, res) => {
  const { courseId, chapterId, currentPage, totalPages, timeSpent } = req.body;
  const documentId = req.params.id;
  const userId = req.user._id;
...
// @route   GET /api/documents/:id/progress
// @access  Private
exports.getDocumentProgress = asyncHandler(async (req, res) => {
  const documentId = req.params.id;
  const userId = req.user._id;
  const { courseId, chapterId } = req.query;
